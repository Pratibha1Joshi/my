...
def decompose(adj, dataset, norm='sym', renorm=True):
    ...
    plt.ylabel('Frequncy')
    plt.savefig("eig_renorm_" + dataset + ".png")
    
def preprocess_graph(adj, layer, norm='sym', renorm=True, subtract_laplacian=True):
    adj = sp.coo_matrix(adj)
    ident = sp.eye(adj.shape[0])
    if renorm:
        adj_ = adj + ident
    ...
    if norm == 'sym':
        ...
        degree_mat_inv_sqrt = sp.diags(np.power(rowsum, -1.).flatten())
        adj_normalized = adj_.dot(degree_mat_inv_sqrt).tocoo()
        laplacian = ident - adj_normalized
    ...
    for i in range(len(reg)):
        if subtract_laplacian:
            adjs.append(ident - (reg[i] * laplacian))
        else:
            adjs.append(ident)  # or adjust based on your specific logic
    return adjs
